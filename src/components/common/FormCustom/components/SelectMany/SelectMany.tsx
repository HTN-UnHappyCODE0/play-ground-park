			...
			>
				...
				{showSelectedItems === 'default' && (
					<div className={styles.list_selected}>
						{selectedItems
							?.map((v) => options.find((f) => getOptionValue(f) === v))
							?.filter(Boolean)
							?.map((selected) => (
								<div
				...
				{showSelectedItems === 'input' && (
					<div className={styles.selected_vertical_wrapper}>
						{selectedItems
							?.map((v) => options.find((f) => getOptionValue(f) === v))
							?.filter(Boolean)
							?.map((selected, index) => (
								<div
									key={getOptionValue(selected!)}
									className={clsx(styles.selected_row, {
										[styles.disabled]: disabledItems.includes(getOptionValue(selected!)),
									})}
								>
									{selectedItemFields?.map((field) => (
										<div key={field.toString()} className={styles.selected_input_wrapper}>
											{index === 0 && (
												<label className={styles.selected_input_label}>
													{selectedItemFieldLabels?.[field.toString()] || field.toString()}
												</label>
											)}
											<input className={styles.selected_input} readOnly value={(selected as any)?.[field] ?? ''} />
										</div>
									))}
									<div className={styles.icon_remove} onClick={() => handleRemoveSelected(getOptionValue(selected!))}>
										<Trash color='#F46161' />
									</div>
								</div>
							))}
			...
			<Popup open={isFocus} onClose={handleClose}>
				<div className={styles.main_popup}>
					...
					<div className={styles.control}>
						<div>
							<Button p_10_40 rounded_50 greyBlue onClick={handleClose}>
								Hủy bỏ
							</Button>
						</div>
						<div>
							<Button disable={listDataTerm?.length == 0} p_10_40 rounded_50 midnightBlue onClick={handleSaveData}>
								Xác nhận
							</Button>
						</div>
					</div>
